...
class Network:
    ...
    def __init__(self):
        ...
        self.model.compile(optimizer=self.optimizer, loss=self.physics_loss, metrics=['mse'])
    ...
    def physics_loss(self, y_true, y_pred):
        # TODO: Make a physics guided loss function
        squared_difference = ops.square(y_true - y_pred)
        loss = ops.mean(squared_difference, axis=-1)
        physics_loss = tf.abs(tf.multiply(tf.reduce_mean(y_pred), self.l) - 1)
        loss += tf.nn.relu(physics_loss - 0.01)
        return loss
    ...
    def generate_data(self):
        ...
        for i in range(10000):
            n = 10
            x = np.random.rand(self.data_points, 1) * self.l
            psi = lambda x: np.sqrt(2 / self.l) * np.sin(x * n * np.pi / self.l)
            x_list.append(x)
